...
export const ingestJob = async (req, res) => {
    try {
        ...
        if (!project) {
            return res.status(404).json({ message: "Project không tồn tại hoặc bạn không có quyền truy cập." });
        }
        ...
        if (existingSnapshot) {
            return res.status(409).json({ message: "Source code này đã được import trước đó (duplicate snapshot)." });
        }
        ...
        blobStream.on("error", (err) => {
            if (!res.headersSent) {
                res.status(500).json({ message: "Lỗi khi upload file lên Firebase: " + err.message });
            }
        });
        ...
        blobStream.on("finish", async () => {
            try {
                ...
                return res.status(200).json({
                    message: "Đã tiếp nhận yêu cầu import source code! Job đang được xử lý.",
                ...
                return res.status(500).json({ message: "Lỗi lưu dữ liệu, đã rollback file upload." });
            }
        });
