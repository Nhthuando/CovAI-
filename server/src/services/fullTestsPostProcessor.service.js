 ...
 * Removes markdown code blocks from the generated full test code.
 */
...
export const generateGenerationSummary = (processedTests) => {
    ...
    return {
        ...
        totalDescribes,
        totalTests,
        message: `Generated full tests for ${totalFiles} files containing ${totalDescribes} test suites and ${totalTests} test cases.`
    };
};

/**
 * SCRUM-446: Main processor function for full tests
 */
export const processFullTests = (tests) => {
    const processedTests = tests.map(test => {
        ...
        if (!isValid) {
            console.warn(`[FullTestsPostProcessor] Test for ${test.filePath} may not be valid Jest syntax.`);
        }
