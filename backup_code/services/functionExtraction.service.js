...
export const extractFunctions = (ast, filePath) => {
    ...
    traverse(ast, {
        FunctionDeclaration(path) {
            functions.push(getFunctionMetadata(path, 'FunctionDeclaration', filePath));
        },
        ArrowFunctionExpression(path) {
            if (processedNodes.has(path.node)) return;
            functions.push(getFunctionMetadata(path, 'ArrowFunctionExpression', filePath));
        },
        FunctionExpression(path) {
            if (processedNodes.has(path.node)) return;
            functions.push(getFunctionMetadata(path, 'FunctionExpression', filePath));
        },
        ClassMethod(path) {
        ...
        ClassProperty(path) {
            if (path.node.value && (path.node.value.type === 'ArrowFunctionExpression' || path.node.value.type === 'FunctionExpression')) {
                ...
                functions.push(getFunctionMetadata(path.get('value'), path.node.value.type, filePath));
            }
        },
        ObjectProperty(path) {
            if (path.node.value && (path.node.value.type === 'ArrowFunctionExpression' || path.node.value.type === 'FunctionExpression')) {
...
const getFunctionMetadata = (path, type, filePath) => {
    ...
    if (type === 'FunctionDeclaration' && node.id) {
        ...
        isAnonymous = false;
    } else if (path.parent.type === 'ClassProperty' || path.parent.type === 'ObjectProperty') {
        name = getNameFromKey(path.parent.key);
        isAnonymous = false;
    } else if (path.parent.type === 'CallExpression' || path.parent.type === 'NewExpression') {
