...
async function getCoverageFilesApi(
  snapshotId,
  { sortBy = "filePath", order = "asc", page = 1, limit = 200 } = {},
) {
...
const OverallRing = ({ value }) => {
  ...
  return (
    <div
      ...
      title="Percentage of code covered by tests"
    >
...
const MetricCard = ({ label, desc, value, color = "#f0f6fc" }) => (
  ...
  >
    ...
    <span
      style={{ fontSize: 24, fontWeight: 700, color, letterSpacing: "-0.02em" }}
    >
      {value}
    </span>
    <span
      style={{ fontSize: 11, color: "#6e7681", marginTop: 4, lineHeight: 1.4 }}
    >
      {desc}
    </span>
  </div>
...
const TestExecutionCard = ({ type, results }) => {
  if (!results)
    return (
      ...
      >
        <span style={{ fontSize: 14, fontWeight: 600, color: "#484f58" }}>
          {type}
        </span>
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ display: "flex", flexDirection: "column" }}>
          <span style={{ fontSize: 20, fontWeight: 700, color: "#f0f6fc" }}>
            {results.totalTests}
          </span>
          <span
            style={{
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          <span
            style={{
              fontSize: 20,
              fontWeight: 700,
              color: results.failedTests > 0 ? "#ef4444" : "#f0f6fc",
            }}
          >
            {results.failedTests}
          </span>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          <span style={{ fontSize: 20, fontWeight: 700, color: "#6e7681" }}>
        ...
        >
          ...
          <span
            style={{
              fontSize: 11,
              fontWeight: 700,
              color: getCoverageColor(passRate).color,
            }}
          >
            {passRate.toFixed(1)}%
          </span>
...
const FileStatusCard = ({ label, count, color, desc }) => (
  ...
  >
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <span style={{ fontSize: 12, color: "#8b949e", marginTop: 4 }}>
        {label}
      </span>
    </div>
  </div>
      ...
      >
        <div
          style={{
            height: "100%",
            width: `${pct ?? 0}%`,
            background: getCoverageGradient(pct),
            borderRadius: 99,
            boxShadow: `0 0 8px ${theme.color}50`,
            transition: "width 0.8s ease-out",
          }}
        />
      </div>
    ...
    >
      ...
      >
        <svg width={18} height={18} viewBox="0 0 14 14" fill="none">
          <path
            d="M2 1.5A.5.5 0 0 1 2.5 1h6L11 3.5V12.5a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-11Z"
            stroke={theme.color}
            strokeWidth={1.5}
          />
          <path
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          {[
            {
              label: "Branches",
              pct: file.branchesPct,
              color: "#38bdf8",
              tooltip: "Percentage of branches (If/Else) executed",
            },
            {
              label: "Functions",
              pct: file.funcsPct,
              color: "#a78bfa",
              tooltip: "Percentage of functions called during tests",
            },
            {
              label: "Statements",
              pct: file.stmtsPct,
              color: "#fb923c",
              tooltip: "Percentage of statements executed successfully",
            },
...
const CoverageDashboard = ({ snapshotId, projectId, onOpenFile }) => {
  ...
  const [isRunningUnitTest, setIsRunningUnitTest] = useState(false);
  ...
  const handleRunUnitTest = async () => {
    ...
    return;
    /*
    setIsRunningUnitTest(true);
    setTimeout(() => { setIsRunningUnitTest(false); }, 1200);
    */
  };
  ...
  const SortIcon = ({ field }) => {
    ...
    return (
      <span style={{ color: "#a78bfa" }}>{order === "asc" ? "↑" : "↓"}</span>
    );
  };
  ...
  if (!summary)
    return <ErrorState message="No data available. Please run tests first." />;
  ...
  const warnCount = files.filter(
    (f) => (f.linesPct ?? 0) >= 70 && (f.linesPct ?? 0) < 90,
  ).length;
    ...
    >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              ...
              disabled={!snapshotId || isRunningUnitTest}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 6,
                ...
                fontSize: 12,
                fontWeight: 600,
                cursor: isRunningUnitTest ? "wait" : "pointer",
                padding: "7px 14px",
                fontFamily: "inherit",
                opacity: !snapshotId || isRunningUnitTest ? 0.65 : 1,
              }}
            >
              {isRunningUnitTest ? "Running Unit Test..." : "Run Unit Test"}
            </button>
            <button
              onClick={handleRunSupertest}
              disabled={!snapshotId || isRunningSupertest}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 6,
                ...
                fontSize: 12,
                fontWeight: 600,
                cursor: isRunningSupertest ? "wait" : "pointer",
                padding: "7px 14px",
                fontFamily: "inherit",
            ...
            <button
              ...
              disabled={!snapshotId || isRunningSystemTest}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 6,
                ...
                fontSize: 12,
                fontWeight: 600,
                cursor: isRunningSystemTest ? "wait" : "pointer",
                padding: "7px 14px",
                fontFamily: "inherit",
                opacity: !snapshotId || isRunningSystemTest ? 0.65 : 1,
              }}
            >
              {isRunningSystemTest
                ? "Running System Test..."
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Score based on the percentage of code lines executed during tests.{" "}
              {cov.lines >= 80
                ? "The project is in a safe state."
                : cov.lines >= 50
                  ? "More tests needed for better safety."
                  : "Code severely lacks tests!"}
            </p>
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <FileStatusCard
                ...
                count={highCount}
                color="#22c55e"
                desc="Coverage above 90%"
              />
              <FileStatusCard
                label="Warning"
                count={warnCount}
                color="#f59e0b"
                desc="Coverage between 70% and 90%"
              />
              <FileStatusCard
                label="Critical"
                count={lowCount}
                color="#ef4444"
                desc="Coverage below 70%"
              />
            </div>
          </div>
        </div>
        ...
        >
          <MetricCard
            ...
            desc="Percentage of branches (If/Else) executed"
            value={fmt(cov.branches)}
            color="#38bdf8"
          />
          <MetricCard
            label="Functions"
            desc="Percentage of functions called during tests"
            value={fmt(cov.functions)}
            color="#a78bfa"
          />
          <MetricCard
            label="Statements"
            desc="Percentage of statements executed successfully"
            value={fmt(cov.statements)}
            color="#fb923c"
          />
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {[{ label: "Coverage Level", field: "linesPct" }].map(
            ({ label, field }) => (
              <button
                key={field}
                ...
                style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  color: sortBy === field ? "#a78bfa" : "#6e7681",
                  textTransform: "uppercase",
                  letterSpacing: "0.08em",
                  cursor: "pointer",
                  display: "flex",
            ...
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: "1px solid rgba(255,255,255,0.08)",
                  borderRadius: 6,
                  color: page >= pagination.totalPages ? "#484f58" : "#c9d1d9",
                  fontSize: 12,
                  cursor:
