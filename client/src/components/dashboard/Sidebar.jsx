...
function EntryActionDropdown({ action, setAction, onSubmit }) {
  ...
  return (
    <motion.form
      ...
      onSubmit={onSubmit}
      className="mx-2 my-1 overflow-hidden rounded-md p-2"
      style={{
        background: "#1c2128",
    ...
    >
      {isDelete ? (
        <>
          <p className="m-0 mb-2 text-xs" style={{ color: "#e6edf3" }}>
            Xóa {action.node.type === "folder" ? "thư mục và nội dung" : "file"}{" "}
            <strong>{action.node.name}</strong>?
          </p>
        ...
        <>
          <input
            ...
            value={action.value}
            onChange={(e) =>
              setAction((current) => ({ ...current, value: e.target.value }))
            }
            placeholder={action.type === "file" ? "new-file.js" : "new-folder"}
...
function TreeNode({
  ...
  return (
    <div>
      ...
      <AnimatePresence>
        {action &&
          (action.node?.id === node.id || action.parentPath === node.id) && (
            <EntryActionDropdown
              action={action}
              setAction={setAction}
...
export default function Sidebar({
  ...
  const requestRename = (node) => {
    ...
    const parent = node.id.includes("/")
      ? node.id.slice(0, node.id.lastIndexOf("/"))
      : "";
    onRenameEntry(
      node.id,
      parent ? `${parent}/${newName.trim()}` : newName.trim(),
    );
  };
  const requestDelete = (node) => {
    ...
    if (
      window.confirm(
        `Xóa ${node.type === "folder" ? "thư mục và toàn bộ nội dung" : "file"} “${node.name}”?`,
      )
    )
      onDeleteEntry(node);
  };
  ...
  const submitAction = async (event) => {
    ...
    if (action.type === "delete") {
      ...
      const parent = action.node.id.includes("/")
        ? action.node.id.slice(0, action.node.id.lastIndexOf("/"))
        : "";
      await onRenameEntry(
        action.node.id,
        parent ? `${parent}/${action.value.trim()}` : action.value.trim(),
      );
    } else if (action.value.trim()) {
      const targetPath = action.parentPath
        ? `${action.parentPath}/${action.value.trim()}`
        : action.value.trim();
      ...
      <AnimatePresence>
        {action && !action.node && !action.parentPath && (
          ...
          >
            {action.type === "delete" ? (
              <>
                <p className="m-0 mb-3 text-xs" style={{ color: "#e6edf3" }}>
                  Xóa{" "}
                  {action.node.type === "folder"
                    ? "thư mục và toàn bộ nội dung"
                    : "file"}{" "}
                  <strong>{action.node.name}</strong>?
                </p>
                <div className="flex justify-end gap-2">
              ...
              <>
                ...
                <input
                  ...
                  placeholder={
                    action.type === "file" ? "src/new-file.js" : "components"
                  }
                  className="w-full rounded px-2 py-1.5 text-xs outline-none"
                  style={{
                    background: "#0d1117",
            ...
            >
              <div style={{ padding: "24px 24px 16px" }}>
                ...
                >
                  ...
                  ? All associated files, snapshots, and analysis data will be
                  lost.
                </p>
